...
class TestBenchmarkJsonSchema:
    ...
    def test_stability_section_schema(self, bench):
        ...
        for variant in ("raw", "one_euro"):
            ...
            assert np.isfinite(entry["mean_iou"])
            assert np.isfinite(entry["centroid_drift"])
        improvement = stability["improvement_pct"]
        assert np.isfinite(improvement["mean_iou"])
        assert np.isfinite(improvement["centroid_drift"])
